...
## TODO 364: Create a database in postgresql in the code line below, and fill in your 
#app's database URI. It should be of the format: postgresql://localhost/YOUR_DATABASE_NAME

## Your final Postgres database should be your uniqname, plus HW5, e.g. "jczettaHW5" or 
#"maupandeHW5"
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/addisonvHW5db"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    newPriority = IntegerField("Enter your new TODO priority", validators = [Required()])
    submit = SubmitField('Update')
...
# TODO 364: Update the all_lists.html template and the all_lists view function such that 
#there is a delete button available for each ToDoList saved.

# When you click on the delete button for each list, that list should get 
#deleted -- this is also addressed in a later TODO.
...
# TODO 364: Update the one_list view function and the list_tpl.html view file so that 
#there is an Update button next to each todolist item, and the priority integer of that 
#item can be updated. (This is also addressed in later TODOs.)

# HINT: These template updates are minimal, but that small update(s) make(s) a big change 
#in what you can do in the app! Check out the examples from previous classes for help.
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        p = TodoItem.query.filter_by(id = item).first()
        p.priority = new_priority
        db.session.commit()
        flash("Update priority of " + item)
        return redirect(url_for('all_lists'))
    return render_template('update_item.html', form = form, item = item)  
    # This code should use the form you created above for updating the specific item and 
    #manage the process of updating the item's priority.
...
# TODO 364: Fill in the update_item.html template to work properly with this update route. 
# (HINT: Compare against example!)

# TODO 364: Complete route to delete a whole ToDoList
@app.route('/delete/<lst>',methods=["GET","POST"])
def delete(lst):
    print(lst)
    current_lst = TodoList.query.filter_by(title = lst).first()
    ...
    # HINT: Compare against what you've done for updating and class notes -- the goal here 
    # is very similar, and in some ways simpler.
